...
description: Integrate Auth0 authentication and authorization services with Pucora API Gateway for secure and seamless access control to your APIs
weight: 100
menu:
  community_current:
...
images:
- /images/documentation/auth0/pucora-auth0-integration-workflow.png
...
The good news is that combining products like Auth0 and Pucora helps you solve this exact problem with an implementation that is straightforward.
...
![Pucora-Auth0 validation](/images/documentation/auth0/pucora-auth0-integration-workflow.png)
...
- With the token generated by Auth0, the client passes it to Pucora in each request inside an HTTP header or cookie
- Pucora authorizes or not the usage of the specific endpoint according to the rules you have configured.

As Pucora can validate the Auth0 signature by itself, it does not need to call an Auth0 server to validate the token every time. Instead, Pucora queries Auth0 every 15 minutes (configurable) to ensure the signature has not rotated.

## Enabling JWT validation in Pucora

We will create a simple Pucora configuration with a single endpoint `/auth0-protected`. This endpoint enables JWT validation and makes sure that no user or machine can access the resource without passing a valid token.

Paste the following configuration into a `pucora.json` file.
...
![Applications view](/images/documentation/auth0/pucora-auth0-integration-application.png)
...
![APIs view](/images/documentation/auth0/pucora-auth0-integration-api.png)
...
```json
{
  ...
  "jwk_url": "https://pucora-auth0-test.auth0.com/.well-known/jwks.json"
}
```
...
Let's open a terminal now and from the folder where we create our `pucora.json` file we run:
{{< terminal title="Start the gateway with your configuration">}}
docker run --rm -v "$PWD:/etc/pucora" -p "8080:8080" {{< product image >}}:{{< product latest_version >}}
{{< /terminal >}}
...
Here it is, that was expected. The gateway won't let us in without a valid JWT token. If you check the Pucora logs, you will also find a line `Error #01: Token not found`.
...
curl --request POST \
  --url https://pucora-auth0-test.auth0.com/oauth/token \
...
- Create **Roles** or **Auth Pipeline Rules** in the Auth0 panel and add them as a condition to accessing an endpoint in Pucora.
...
Combining both systems provides excellent benefits. For one side, Auth0 lets you forget about the arduous task of managing, auditing, securing, or updating standards that an **authentication** system requires. For the other, Pucora handles the complex part of **authorizing** users, which requires a high level of security, such as dealing with fingerprints and cipher suites while using an approach that allows you to scale linearly ingesting any amount of traffic.

Auth0 and Pucora collaborate efficiently and without coupling during the run-time to reliably identify your users and let them see what they are supposed to.
