...
description: Learn how to integrate Descope authorization with Pucora API Gateway for secure and controlled access to your APIs.
weight: 110
menu:
  community_current:
...
Securing your APIs and underlying microservices is crucial in modern development. Descope enables you to add advanced authentication capabilities to your Pucora endpoints efficiently.
...
- With the token generated by Descope, the client passes it to Pucora in each request inside an HTTP header or cookie
- Pucora authorizes or not the usage of the specific endpoint according to the rules you have configured.

As Pucora can validate the Descope signature by itself, it does not need to call the Descope server to validate the token every time. Instead, Pucora queries Descope every 15 minutes (configurable) to ensure the key has not rotated.

## Protecting endpoints with Descope tokens

We will create a simple Pucora configuration with a single endpoint `/descope-protected`, ensuring only users with valid tokens can access it.

Create a `pucora.json` file and add the following configuration:
...
From the folder where we create our `pucora.json` file, start the gateway with:
{{< terminal title="Start the gateway with your configuration">}}
docker run --rm -v "$PWD:/etc/pucora" -p "8080:8080" {{< product image >}}:{{< product minor_version >}}
{{< /terminal >}}
...
Since no token is provided, Pucora correctly denies access. If you check the Pucora logs, you will also find a line `Error #01: Token not found`.
...
- Create [**Roles** in Descope](https://docs.descope.com/authorization) and add them as a condition to accessing an endpoint in Pucora.
...
For more details, see [Pucora's JWT Validation documentation](/docs/authorization/jwt-validation/).

### Conclusion
Integrating Descope with Pucora enhances security while maintaining flexibility. You can designate protected and public endpoints, ensuring controlled access to your APIs. Descope’s seamless integration with Pucora streamlines authentication and improves user experience, making it an excellent choice for secure API management