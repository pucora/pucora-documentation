...
description: Learn how to extend the functionality of Pucora API Gateway by writing and building custom plugins, enabling custom business logic and workflows
weight: 10
...
Plugins are **an independent binary** of your own and are not part of Pucora itself. Plugins allow you to "*drag and drop*" (so to speak) custom functionality that interacts with Pucora while still using the official binaries without needing to fork the code.
...
1. A user or machine sends an HTTP request to Pucora. The initial hit is processed by the `router pipe` that decides what to do with it.
...
| -------- | ------- | ------- |
| {{< badge color="#6f00f0">}}server{{< /badge >}}      | Router | [HTTP server plugins](/docs/extending/http-server-plugins/) modify the HTTP request and response between the end-user and Pucora (the server)    |
...
| {{< badge color="#e900b7">}}client{{< /badge >}}      | Transport | [HTTP client plugins](/docs/extending/http-client-plugins/) modify the request and response between Pucora and the upstream services (internal HTTP client)    |
...
If you have gone through the different functionalities of Pucora and think that a combination of components does not fulfill your needs, writing a plugin can be the solution. This document omits the initial parts of the development lifecycle (plan, analyze, design...) and jumps directly to the **implementation part**.
...
- **Same Go version**: Your plugin and Pucora are compiled with the same Go version. E.g., you cannot build a plugin on Go 1.19 and load it on a Pucora assembled with Go 1.22. The `pucora version` tells you the Go and Glibc versions.
- **Same architecture/platform**: Pucora and plugins must have been compiled in the same architecture. E.g., you cannot compile a plugin in a Mac natively and use it in a Docker container
- **Same shared library versions**: If the Pucora core also uses external libraries, they must import identical versions.
- **Injection in the configuration**: Besides coding and compiling your plugin, you must add it to the `pucora.json` configuration.

Yes, it sounds rigorous, but fortunately, Pucora has developed many tools, so you don't have to spend time thinking about this. Let's see them below.
...
Pucora CE users create a Go file and implement the interface for the chosen plugin type, as shown in each plugin guide.

When the interface is correct, implement the rest of the custom logic you'd like to have.

## Check the dependencies
Before compiling the plugin, you must ensure the libraries you use in your code are compatible with Pucora. To do so, execute the command [`{{< product check_plugin_command >}}`](/docs/extending/check-plugin/) that analyzes your `go.sum` file and warns you about incompatibilities.

This is a crucial step because your custom plugins need to match the Go and library versions used to build Pucora
...
When using Docker to deploy your gateway, our official Pucora container uses **[Alpine](https://hub.docker.com/_/alpine)** as the base image. Therefore, to use your custom plugins, they must compile using the Alpine builder.
...
The command will generate a `yourplugin.so` file (name it as you please) that you can now copy into a `{{< product image >}}:{{< product latest_version >}}` Docker image, and load it as described in [injecting plugins](/docs/extending/injecting-plugins/). **Never use a tag that mismatches the builder and Pucora**. If you want to load the plugin in a Pucora version `x.y.x` make sure to build it on a builder `x.y.z`. Using `.so` files that were compiled in a builder with a different version, will mostly fail.
...
## Test the plugin
Once your `.so` file is available, you must check that the plugin is loadable by Pucora. You can test the plugin using the [test command](/docs/extending/test-plugin/)
...
## Inject your plugin and run Pucora
The final step before running the plugin is including the configuration of the new plugin.

To inject a plugin, you must copy it into a Pucora directory and add it to the `plugin` configuration at the service level of your configuration. Then, add your plugin namespace where necessary across the configuration.

When you run Pucora, the plugin shows as loaded in the log.
