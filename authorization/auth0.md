...
description: Integrate Auth0 authentication and authorization services with Velonetics API Gateway for secure and seamless access control to your APIs
weight: 100
menu:
  community_current:
...
images:
- /images/documentation/auth0/velonetics-auth0-integration-workflow.png
...
The good news is that combining products like Auth0 and Velonetics helps you solve this exact problem with an implementation that is straightforward.

Auth0 can authenticate users and machines, and Velonetics knows your business APIs and validate them in a very efficient and convenient way. Both products work in perfect symbiosis to secure your APIs.

## The Auth0 - Velonetics integration
JSON web tokens (JWT) are an open standard ([RFC 7519](https://datatracker.ietf.org/doc/html/rfc7519)) that define a way for securely transmitting information between parties. JWT tokens are compact, secure (digitally signed), and have become an industry-standard used at internet scale. Velonetics supports [any system using this open standard](https://www.velonetics.io/docs/authorization/jwt-validation/), and Auth0 has been its preferred SaaS integration.

Velonetics offers integration with Auth0 at three different levels:
...
![Velonetics-Auth0 validation](/images/documentation/auth0/velonetics-auth0-integration-workflow.png)
...
- With the token generated by Auth0, the client passes it to Velonetics in each request inside an HTTP header or cookie
...
```json
{
  ...
  "jwk_url": "https://velonetics-auth0-test.auth0.com/.well-known/jwks.json"
}
```
...
Let's open a terminal now and from the folder where we create our `velonetics.json` file we run:
{{< terminal title="Start the gateway with your configuration">}}
docker run --rm -v "$PWD:/etc/velonetics" -p "8080:8080" {{< product image >}}:{{< product latest_version >}}
{{< /terminal >}}
...
Here it is, that was expected. The gateway won't let us in without a valid JWT token. If you check the Velonetics logs, you will also find a line `Error #01: Token not found`.
...
curl --request POST \
  --url https://velonetics-auth0-test.auth0.com/oauth/token \
...
- Create **Roles** or **Auth Pipeline Rules** in the Auth0 panel and add them as a condition to accessing an endpoint in Velonetics.
- Define scopes and validate them on Velonetics
- Propagate claims to your backends.

All of this is possible by [declaring more elements into the configuration](https://www.velonetics.io/docs/authorization/jwt-validation/).

### Conclusion
In this walk-through, we have shown how to securely protect your APIs using Velonetics and Auth0 in a few minutes.

Combining both systems provides excellent benefits. For one side, Auth0 lets you forget about the arduous task of managing, auditing, securing, or updating standards that an **authentication** system requires. For the other, Velonetics handles the complex part of **authorizing** users, which requires a high level of security, such as dealing with fingerprints and cipher suites while using an approach that allows you to scale linearly ingesting any amount of traffic.

Auth0 and Velonetics collaborate efficiently and without coupling during the run-time to reliably identify your users and let them see what they are supposed to.
