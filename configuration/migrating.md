...
title: Upgrading Velonetics from an older version
description: Seamlessly migrate your Velonetics configuration to a new version. Hassle-free transition for your API Gateway
menu:
  community_current:
    parent: "010 Configuration files"
weight: 1000
---
Upgrading to a new version of Velonetics is designed to be straightforward, thanks to our commitment to **maintaining backward compatibility** across versions within the same major release (e.g., within `2.x` versions). You can generally update Velonetics to a newer version without altering your configuration. However, to ensure optimal performance and access to the latest features, reviewing and adjusting your configuration is wise.

{{< note title="Velonetics's upgrade policy" type="info" >}}
Our policy focuses on **minimizing disruption by preserving compatibility with previous versions**. However, important changes can occur between versions, such as the deprecation of components, the introduction of superior alternatives, the relocation of properties, or the outright removal of outdated features. Although Velonetics aims to ensure your existing setup will continue to run, these changes may necessitate adjustments to your configuration for improved stability and performance.
{{< /note >}}
...
2. **Adjust your configuration and run the linter** as needed. Below, you'll find the changes between versions. Scroll down to your current version and apply all changes above it. Then, run [the linter](/docs/configuration/check/) (`velonetics check --lint`), which is designed to scrutinize your configuration rigorously.
3. **Update the Velonetics binary**. Replace the existing binary file with the latest version. This process varies depending on how Velonetics was installed and whether it is container-based or not.
...
The Velonetics 2.0 release is a major version that **simplifies the configuration** of `v1.x` and **standardizes field names** that were using different criteria to declare the attributes. The migration tool allows you to migrate from Velonetics `0.x` or `1.x` to `2.0`

{{< button-group >}}
{{< button url="https://github.com/velonetics/velonetics-config-migrator" text="Download migration tool" >}}<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>
{{< /button >}}
{{< /button-group >}}
...
- Download the configuration [migration tool](https://github.com/velonetics/velonetics-config-migrator) and execute it passing the path to your Velonetics project
- Review the changes the migration tool did to your config and start the config with the new version

**If you have custom go plugins**, recompile them. Velonetics has now a command [`velonetics check-plugin`](/docs/extending/check-plugin/) and [`velonetics test-plugin`](/docs/extending/test-plugin/) to test them.

{{< note title="Special attention to short words" >}}
The migration script replaces words used by Velonetics in the past and are no longer supported that might collide with wording you use in your endpoints. Words like `whitelist` or `blacklist` will be replaced by `allow` and `deny`. Make sure to check the changes in the configuration and ensure that the migration tool didn't change any endpoint definition using those names.
{{< /note >}}
...
```json
{
    "endpoint": "/foo",
    "extra_config": {
        "github.com/velonetics/velonetics-ce-jose/validator" {
            "alg": "RS256",
