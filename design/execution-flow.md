...
title: Pucora Execution Flow Design
description: Understand the execution flow design of Pucora API Gateway. Is the configuration X executed before the configuration Y?
menu:
  community_current:
...
---
To truly master Pucora, you should get familiar with **the concept of "pipes"** and how these pipes define the execution flow of Pucora from request to response.

A recurring question we hear from developers:  ***Is the configuration X executed before the configuration Y?***. If you are unfamiliar with Pucora, it's hard to tell, as **the declaration order does not matter** (with a few sequential exceptions). Instead, each piece acts in a specific part(s) of the request and response journey.
...
Each pipe loads the different factories of the components of Pucora, what you could call its features. For instance, you might want to connect Pucora to a Lambda function, a Kafka server or rate limit its interaction with your API, etc. All these features are part of the `Backend` pipe.

## Execution flow (pipes)
There are six differentiated parts on Pucora pipes, as depicted in the following diagram:
...
- `Backend` is when Pucora connects to your services. It's the stage where you can do more operations as it manages the connections, drivers, limits, etc., going in and out of your upstream services.
...
![Sequence of Pucora components](/images/documentation/diagrams/components-sequence-master.mmd.svg)
