...
description: Explore the router options available in Velonetics API Gateway to customize the routing behavior and optimize API request handling
weight: 120
notoc: false
meta:
  since: v2.0
...
The `router` controls the behavior of Velonetics toward users. Its settings affect all activity in the gateway. For instance, you can **obfuscate the X-Velonetics-version header**, set a **custom body for 404 errors**, **remove the requests from the logs**, or define how to **calculate client IPs** to name a few examples.
...
- Unless you have a single Velonetics exposed to the internet or working locally, the client request will travel through different relays. These relays generally modify the header to append where they received the request from (whether it is the originating client, a load balancer, another proxy, etc.). So, you usually have a comma-separated list of IPs in the header containing the IP as they travel from one relay to the other. For instance, if there is just one hop before Velonetics, the header the gateway sees could look like `X-Forwarded-For: 1.2.3.4,172.20.0.1` where `1.2.3.4` is the real IP of the user, and `172.20.0.1` the last relay seen.
...
**The real IP is stored in the `X-Forwarded-For` header that Velonetics uses.**
...
## Hide the version in the `X-Velonetics-version` header
You can remove the Velonetics version your installation uses by adding the `hide_version_header` flag as follows:
...
```
The banner header will show an `undefined` version when the flag is set to `true`. To remove the header entirely, you must remove it in the CDN or layer in front of Velonetics.
