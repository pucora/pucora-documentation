...
description: Overview of the options available to integrate authentication and authorization services with Pucora API Gateway for secure access control to your APIs
weight: -100
menu:
  community_current:
...
Pucora offers a versatile set of features enabling you to control who can access your APIs and what actions they are authorized to perform. Whether your use case involves simple client credentials or complex multi-provider authentication scenarios, Pucora ensures your system is both secure and scalable.

**This introduction provides an overview of Pucora's key authorization and authentication features.**
...
In addition, the Pucora adds other methods like Google Cloud service-to-service or more "classic" (and less secure) authentication methods, such as API-key authentication, Basic Authentication, or even Microsoft's NTLM (NT Lan Manager) Authentication.

## Authorization (AuthZ)
Authorization determines what authenticated users or systems are **permitted** to do. Pucora provides flexible options for fine-grained access control. The most common method is Token-based authorization (see below). In addition to JWT, you can also use OAuth2 Client Credentials for machine-to-machine communication. Pucora verifies the client's credentials with this flow and enforces scope-based access controls.

Pucora's ability to integrate with multiple identity providers simultaneously offers great flexibility. Whether catering to different user groups, migrating, multi-tenant, or implementing complex organizational structures, Pucora adapts to your needs.

## Token-based authorization
Today, most applications use JSON Web Tokens (JWTs), a widely used method for securing APIs. They are our first recommended way to limit the access of end-users to your API. Pucora does not issue the JWT tokens (as we said, this is the job of the identity provider) but validates them to ensure their authenticity and can check claims, such as audience, issuer, and expiration, to name a few examples.

Read the [JWT Overview](/docs/authorization/jwt-overview/) for an introduction to token-based authorization in Pucora. When working with tokens, there are different things you can do:

- [JWT Signing](/docs/authorization/jwt-signing/): If you don't yet have an Identity Provider, you can make your classic login controller return a JSON response emulating a token and let Pucora sign it with secure cryptography. This enables your system to provide trusted, verifiable tokens for authenticated clients even when you have not transitioned your users to an identity service.
- [JWT Validation](/docs/authorization/jwt-validation/): This is the core functionality, where a user sends a token generated by an Identity Provider, and Pucora checks for its validity and any required permissions to access the resource.
...
If you have multiple identity providers ( feature), you can let Pucora work with all of them simultaneously.

## Role-Based Access Control (RBAC) and Attribute-Based Access Control (ABAC)
It is important to know who can access a resource and decide if a user has a specific role or attribute that grants them access. The different components of authz in Pucora allow you to set custom rules validated on runtime during **requests, responses, and token validation** itself.
...
Because Pucora adopted security standards, you can use it **with any provider**: we still need to find a single major provider that doesn't follow the JSON Web Encryption (RFC 7516), JSON Web Signature (RFC 7515), or JSON Web Token (RFC 7519) specifications.
...
In this scenario, the `/token` endpoint passes through Pucora, allowing you to set rate limiting or any other protection mechanism you consider to secure even more your IdP. The client stores locally the token for the next interactions, and send the bearer to access resources through Pucora. The IdP is not contacted again until the token is expired (when the client will refresh it).