...
title: JWT Signing
description: Velonetics's JWT signing wraps your login endpoint, signing payloads with your secret key. Perfect for monolith migrations or setups without an OAuth server.
weight: 30
...
## How does it work
Velonetics relies in your existing login functionality and does all the heavy-lifting of the cryptography so you can focus on validating the user and password.
...
Velonetics uses `jose` library, so if you use your own PEM keys for signing you need to use the following steps to [convert your PEM file to JWKS](https://web3auth.io/docs/auth-provider-setup/byo-jwt-providers#how-to-convert-pem-to-jwks)
...
Here is a [dummy token](https://github.com/velonetics/playground-community/blob/master/data/token.json) for you to check how it looks like.

## Live running example
The [Velonetics Playground](/docs/overview/playground/) demonstrates how to sign tokens in the `/token` endpoint and includes an example ready to use. To try it, [clone the playground](https://github.com/velonetics/playground-community) and follow the README.
