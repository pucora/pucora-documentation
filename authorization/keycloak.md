...
description: Learn how to integrate Keycloak authorization with Pucora API Gateway for secure and controlled access to your APIs.
weight: 110
menu:
  community_current:
...
- With the token generated by Keycloak, the client passes it to Pucora in each request inside an HTTP header or cookie
- Pucora authorizes or not the usage of the specific endpoint according to the rules you have configured.

As Pucora can validate the Keycloak signature by itself, it does not need to call the Keycloak server to validate the token every time. Instead, Pucora queries Keycloak every 15 minutes (configurable) to ensure the key has not rotated.

## Protecting endpoints with Keycloak tokens

We will create a simple Pucora configuration with a single endpoint `/keycloak-protected`. This endpoint enables JWT validation and makes sure that no user or machine can access the resource without passing a valid token.
