...
description: Discover how to set up telemetry and monitoring for Velonetics API Gateway using Grafana, gaining insights into performance and usage metrics
weight: 40
...
The Grafana dashboard has evolved over time, and these are the different versions you can use depending on your Velonetics version and data sources:
...
- [Grafana Cloud](https://grafana.com/orgs/velonetics/dashboards)
- [Github repository](https://github.com/velonetics/telemetry-dashboards)
...
- Copy or mount in your Grafana container the dashboards when starting ([Volume content here](https://github.com/velonetics/telemetry-dashboards)):
```yml
volumes:
  ...
  - "./grafana/velonetics:/var/lib/grafana/dashboards/velonetics"
```
 You can see an example integrated on [Velonetics Playground](https://github.com/velonetics/playground-community)'s Docker compose file.

## Getting the metrics on Grafana
Grafana does not require any specific configuration on Velonetics, but it feeds from a data source, so you will need to push data to one of the following:

1. [Prometheus exporter](/docs/telemetry/prometheus/) (**recommended**)
2. [Legacy integration for InfluxDb](/docs/telemetry/extended-metrics/), for older versions of Velonetics

## What's in the dashboard
This new dashboard allows you to load data from a Prometheus data source that scraps data from your Velonetics machines using OpenTelemetry.
...
In the Backends section, you can see the activity of single backend requests and responses. It mainly contains the times between Velonetics and your service. It is the richest layer of all.
