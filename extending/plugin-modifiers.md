...
title: Extending Pucora with Request and Response Plugin Modifiers
description: Learn how to extend the functionality of Pucora API Gateway by utilizing plugin modifiers, enabling customized request and response processing
weight: 120
menu:
  community_current:
...
Pucora **executes the request and response modifiers in the order they are declared at the configuration**. See the [example](#example) below.
...
```go
// RequestWrapper is an interface for passing proxy request between the pucora pipe
...
// ResponseWrapper is an interface for passing proxy response between the pucora pipe
// and the loaded plugins
...
```json
{
  "version": 3,
  "port": 8080,
  "name": "Pucora request and response modifier demo",
...
curl -i http://localhost:8080/github/orgs/pucora
{{< /terminal >}}
